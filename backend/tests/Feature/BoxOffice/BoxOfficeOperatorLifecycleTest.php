...
class BoxOfficeOperatorLifecycleTest extends TestCase
{
    ...
    /**
     * A second event/product/price graph attached to an EXISTING account
     * (createEventWithProduct always spins a fresh account).
     */
    private function createEventWithProductOnAccount(int $accountId): array
    {
        $ownerId = DB::table('account_users')->where('account_id', $accountId)
            ->where('is_account_owner', true)->value('user_id');
        \Illuminate\Support\Facades\Auth::login(\HiEvents\Models\User::find($ownerId));

        $organizer = \HiEvents\Models\Organizer::create([
        ...
        $event = \HiEvents\Models\Event::create([
            'title' => 'Second Event',
            'account_id' => $accountId,
            'organizer_id' => $organizer->id,
            'currency' => 'USD',
            'status' => 'LIVE',
            'short_id' => \HiEvents\Helper\IdHelper::shortId(\HiEvents\Helper\IdHelper::EVENT_PREFIX),
        ]);
        \HiEvents\Models\EventSetting::create([
            'event_id' => $event->id,
            'allow_orders_awaiting_offline_payment_to_check_in' => true,
        ]);
        $product = \HiEvents\Models\Product::create([
            'event_id' => $event->id,
            'title' => 'GA 2',
        ...
        $price = \HiEvents\Models\ProductPrice::create([
